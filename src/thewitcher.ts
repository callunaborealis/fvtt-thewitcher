...
Hooks.once("init", async function () {
  ...
  Handlebars.registerHelper("localizeAbility", (abilityId) =>
    game.i18n.localize(`THEWITCHER.actor.abilities.${abilityId}.label`)
  );
  Handlebars.registerHelper("localizeAvailability", (avail) =>
    game.i18n.localize(`THEWITCHER.item_sheet.availability.${avail}`)
  );
  Handlebars.registerHelper("isResource", (data) => data.hasOwnProperty("max"));
  Handlebars.registerHelper("getAbilitySkills", (data, abilityId) =>
    data.skills.get(abilityId)
  );
  Handlebars.registerHelper("checkboxMultiple", (name, choices, options) => {
    ...
    const option = (key, label, index) => {
      ...
      html += `<label class="checkbox ${
        isSelected ? "checked" : ""
      }">${label} <input type="checkbox" name="${name}.${key}" ${
        isSelected ? "checked" : ""
      } /></label>`;
    };
