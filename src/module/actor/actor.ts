...
export default class WitcherActor<T = {}> extends Actor<T> {
  /** @override */
  prepareBaseData() {}
  ...
  private _prepareCharacterData(actorData: ActorData<any>) {
    ...
    data.abilities_points = Object.values(data.abilities).reduce(
      (t, { value }) => t + value,
      0
    );
    ...
    data.attributes.resolve.disabled = !game.settings.get(
      "thewitcher",
      "verbalCombat"
    );
  }
  ...
  private _prepareDerivedAbilities(
    ...
    let physicalBase = Math.floor(
      (this._getAbilityValue("body") + this._getAbilityValue("will")) / 2
    );
    ...
    data.attributes.leap.max = Math.floor(
      (this._getAbilityValue("spd") * 3) / 5
    );
    data.attributes.enc.value = body * 10;
    data.attributes.resolve.value = Math.floor(
      ((this._getAbilityValue("spd") + this._getAbilityValue("int")) / 2) * 5
    );
  ...
  private _getSkillValue(
    abilityId: string,
    skillId: string,
    with_ability: boolean = false
  ): number {
    const skillData = getProperty(
      this.data,
      `data.skills.${abilityId}.${skillId}`
    );
  ...
  rollAbility(
    ...
    const label = game.i18n.localize(
      `THEWITCHER.actor.abilities.${abilityId}.label`
    );
    const abilityValue = this._getAbilityValue(abilityId);
    ...
    const rollData = mergeObject(options, {
      terms: terms,
      data: data,
      title: game.i18n.format("THEWITCHER.rolls.ability_prompt_title", {
        ability: label,
      }),
  ...
  rollSkill(
    ...
    const rollData = mergeObject(options, {
      terms: terms,
      data: data,
      title: game.i18n.format("THEWITCHER.rolls.skill_prompt_title", {
        skill: label,
      }),
      messageData: {
        "flags.thewitcher.roll": { type: "skill", abilityValue, skillValue },
      },
    });
    // @ts-ignore
    rollData.speaker =
  ...
  rollProfessionSkill(
    ...
    if (!main) {
      skill = getProperty(
        this.data,
        `data.profession.tree.${treeId}.chain.${skillId}`
      );
    }
    ...
    const rollData = mergeObject<Record<string, any>>(options, {
      terms: terms,
      data: data,
      title: game.i18n.format(
        "THEWITCHER.rolls.profession_skill_prompt_title",
        { skill: label }
      ),
      messageData: {
        "flags.thewitcher.roll": {
          type: "profession_skill",
          skill,
          abilityValue,
          skillValue,
        },
